...
import GameValue from '../../../constants/game-values';
...
interface FieldProps {
    onUserChoose: (x: number, y: number) => void;
}

const Field: FC<FieldProps> = ({ onUserChoose }) => {
    // Sound.setCategory('Playback');
    // const turnSound = new Sound(turn);

    const field = useTypedSelector(state => state.game.gameState.field)
    const lastField = useTypedSelector(state => state.game.gameState.lastField);
    ...
    const checkUserChoice = (x: number, y: number) => {
        setTimeout(() => {
    ...
    const createCell = (cellValue: GameValue, row: number, column: number) => {
